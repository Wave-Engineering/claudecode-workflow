...
- **The fleet is now queue-less — no GitHub merge queue, no GitLab merge trains (#898).** The merge-queue rulesets were deleted from all six Wave-Engineering repos (classic branch protection, which independently carried the required status checks, was left intact), and `gl-settings` no longer enables merge trains or merged-results pipelines. **Why:** wave work never needed a queue — flights land on the `kahuna/*` integration branch, the engine reconciles them via `commutativity_verify` + dependency-ordered merges, and `kahuna→main` is a single serialized, trust-gated promotion, so there is no concurrent-merge-to-protected-branch point for a queue to guard. Meanwhile the queue cost **3 pipelines per MR** on GitLab and caused the `skip_train`-silently-dropped divergence bug and the 2026-04-07 `merge_group:`-missing outage (postmortem #299). A prior spec claim that merge trains "batch flight-MRs into one pipeline run" was **false** — GitLab runs a pipeline *per MR in the train* — and is corrected at the source in `docs/kahuna-devspec.md`. Merge method is now a **per-merge** choice (squash for normal PRs, merge-commit for the wave promote), which is only possible without a queue forcing one method for everything. `docs/operations/merge-queue-checklist.md` → `docs/operations/branch-protection-checklist.md`, rewritten around the queue-less config while **preserving** the load-bearing "config exists ≠ config works" discipline (now: a red PR must be BLOCKED *and* a green PR must merge — a gate that blocks everything is as broken as one that blocks nothing). The dead `merge_group:` trigger is removed from `validate.yml`. The old kit doc is pruned from `~/.claude` via `DEPRECATED_PATHS` — it *prescribed* creating a merge queue, and repos without their own docs resolve kit docs from `~/.claude`.
