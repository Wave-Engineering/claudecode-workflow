...
| R-08 | Event-driven | When `wave-status record-mr <issue> <mr>` is called, the system shall record the MR/PR reference for the issue in its OWN wave's `mr_urls` (not necessarily `current_wave` — a straggler MR recorded after the campaign has advanced targets the issue's real wave, cc-workflow#1158) and regenerate the dashboard. Falls back to `current_wave` when the issue isn't in the plan, the plan file is absent, or the resolved wave isn't a real wave in state. When `current_wave` is itself `None` (the terminal wave already completed, cc-workflow#1161), resolves via the plan the same way; errors only when neither `current_wave` nor a plan-resolved wave is available. |
