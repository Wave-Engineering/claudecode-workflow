...
- **Evidence:**
  ...
  - CI wiring: `scripts/ci/validate.sh` lines 247–* iterate `tests/regression/*.sh`; `.github/workflows/validate.yml` runs `./scripts/ci/validate.sh` on every PR to main — so the regression is gated on all future merges including the closing kahuna→main MR.
- **Result:** PASS.
