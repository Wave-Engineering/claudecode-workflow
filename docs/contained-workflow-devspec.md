...
| R-07 | mechanical promotion gate | Story 2.3 | E2E-02, test_gate_conjunction, test_soak_bridge | e2e / unit | **Partial** — gate conjunction unit-verified (test_gate_conjunction green); auto-soak accrual now **WIRED** via the bridge (surgeon `--live` → `soak_ledger.accrue`, `scripts/ci/soak-accrual-bridge.sh`, hermetically proved by test_soak_bridge, **#1008**); the live E2E-02 cycle (real `:edge` ring → gate soak fills → promote) is still **operator-run** (MV-04/MV-06) — not claimed Verified |
| R-08 | rolling adoption | Story 2.4 | E2E-02 | e2e | **Partial** — adoption oracle green (unit); auto-soak accrual now wired (bridge, **#1008**), so the soak leg of E2E-02 is no longer a gap; the live E2E-02 cycle itself is still deferred to the operator field-run |
| R-09 | MCP additive scoping | Story 1.3 | IT-03 | integration | **Verified — both halves.** Additive-scoping half via the mount manifest + resolver (`mounts.d/`, `mount_resolver.py`). Kit-MCP-baking half delivered in **#1013**: the kit MCP repos are all PUBLIC (no build secret, no operator step), so `./install` (no `--no-mcps`) bakes all five kit MCP servers — disc-server, discord-watcher, nerf-server, sdlc-server, wtf-server — each registered in the uid-1000 user's `~/.claude.json` `.mcpServers` and dropped as a prebuilt linux-x64 binary in `~/.local/bin`. Proven by a real `docker build` (assertion in the build itself) and by `tests/contained-workflow/test_image.py::test_image_kit_mcps_baked`, which CI's `throwaway-ci-ring.sh` runs against the pushed digest (`OAKANDWAVE_REQUIRE_IMAGE=1`). sdlc-server's bundled commutativity-probe (Python CLI) is now baked too (**#1014**): its `install-remote.sh` pip/pipx/venv paths all no-op because the base python lacks pip/ensurepip, so the Dockerfile installs it via `uv tool install` (pinned `SDLC_PROBE_REF`, default `v0.1.0`) — `uv`'s own build frontend needs no system pip and drops the `commutativity-probe` console script into the uid-1000 user's `~/.local/bin`, where sdlc-server resolves it; `commutativity_verify` no longer degrades to `PROBE_UNAVAILABLE` in-container. Asserted by `test_image.py::test_image_commutativity_probe_baked`. |
