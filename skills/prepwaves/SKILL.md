...
7. **Persist.** Before assembling the final `plan_json`, set its **plan identity** (cc-workflow#1171): a top-level `plan_id` (the FIRST Plan tracking-issue number passed to `/prepwaves` in step 2 — the same one across every Phase, even for a multi-Plan `/prepwaves #2 #3` run) and a `slug` (kebab-case, `^[a-z0-9][a-z0-9-]*$` — derive from that Plan issue's title: lowercase, non-alphanumerics to hyphens, collapse repeats, trim to ~40 chars; **must match `/devspec upshift`'s own slug for the same Plan** if one already exists, since the same value feeds both `kahuna/<plan_id>-<slug>` and `campaign/<plan_id>-<slug>` branch names elsewhere — check for an existing slug on the Plan issue/persisted plan before deriving a fresh one). This is the exact `plan_id`/`slug` shape `/devspec upshift` already writes (`skills/devspec/SKILL.md` Step 6) — reusing its field names, not inventing new ones.

   Call `wave_init(plan_json)` — the tool auto-detects existing plans and uses extend mode, preserving completed waves. Use Phase-prefixed wave IDs (e.g., `wave-2a`) to avoid collisions when extending. Cross-repo fields (`cross_repo`, `target_repos`) round-trip without modification on extend (the underlying `wave-status init` writes the plan dict verbatim to `phases-waves.json`) — but **`plan_id`/`slug` do NOT**: `extend_state` (`state.py`) merges only the incoming `phases` array into the EXISTING persisted dict and re-saves that dict, so any top-level key on a FRESH init persists verbatim, but on an EXTEND any top-level key not already in the existing file is silently dropped, no error. The per-wave `dispatch` field (step 4.A) round-trips on fresh init the same way `plan_id`/`slug` do; its READING and *enforcement* at execution time is `/nextwave`'s job, delivered by **Story 1.2 (#824)**: a wave (or an older `phases-waves.json`) that carries **no** `dispatch` field will be treated as `serialize` — a backward-compatible default. Until #824 lands, `/prepwaves` only *writes* the annotation; the executor's dispatch enforcement (in `per-wave-workflow.js`) is #824's deliverable, not this story's.

   **Pin the FlightDeck campaign card (cc-workflow#1171).** Immediately after `wave_init` returns `ok`, resolve the activity id from the FILE JUST WRITTEN, not from this invocation's own local variables — this is what guarantees `/prepwaves`'s `campaign-head` call and `/nextwave`'s later `args.planId` read (step 1) agree on the same value, closing the exact "one campaign, two activity ids" split flightdeck#1144 already hit once:

   1. Read `plan_id` back from the just-persisted plan file (the same file the "Commit the plan file" step below operates on).
   2. **If present** (the normal case: fresh init, or an extend of a plan that already carried one) — use it directly.
   3. **If absent** (an extend of a LEGACY plan persisted before this convention existed — `extend_state`'s drop behavior above means `wave_init` alone can never backfill it) — this is not a silent skip: directly patch the persisted file (read the JSON, add `plan_id`/`slug`, write it back — the same verbatim round-trip `wave_init` itself uses, just performed once, outside it) so this campaign gets a stable identity going forward. Report the backfill in this step's summary; it is a one-time correction to a plan predating #1171, not routine behavior.
   4. Call, non-blocking (same idempotent pattern `/wavemachine`'s own launch sequence uses, `skills/wavemachine/SKILL.md` "Launch sequence"):
      ```bash
      wave-status campaign-head --activity-id "<plan_id from step 1-3 above>" || true
      ```

   `campaign-head` derives `planTotal`/`workItemsTotal`/`waveWorkItems`/the project label FROM THE JUST-PERSISTED PLAN (`resolve_campaign_head_detail` reads `phases-waves.json`) — never a hand-typed literal — and refuses loudly rather than emitting a card with a guessed total if the plan can't be read (`|| true` only protects this launch step from aborting on that refusal, it does not hide the message — report it, don't swallow it). Without this call, a plan executed via a standalone `/nextwave` (no `/wavemachine` wrapper) never declares `activityType: campaign` on any event, and flightdeck's fold.ts (flightdeck#31/#42) leaves its card in the "headless" UI bucket forever — no progress fraction, no ETA — even though cc-workflow#1170 already gives it a real head row. Calling it here, once, right after persist, closes that gap for BOTH invocation paths (orchestrated and standalone) from the one place both of them go through. Idempotent: a second call from `/wavemachine`'s own launch sequence later (same campaign, same plan_id) just re-declares the same values — not a conflict (flightdeck#41's `activityTypeConflict` only fires on a DIFFERING declaration).

   **Side effect worth knowing, not a defect:** `campaign-head` writes the durable FlightDeck scope marker (`emit.py`'s campaign allowlist) at *plan* time now, not just at `/wavemachine` *launch* time. Any `wave-status`/sdlc-server call in this checkout between `/prepwaves` and the eventual `/nextwave`/`/wavemachine` run — unrelated PR work, another agent sharing the clone — is scoped to this campaign until an `activity_end` clears it. Usually seconds (step 9 recommends a fresh session before running the campaign); the window is only unbounded if a plan is prepped now and run much later.

   **Requires `wave-status` on PATH** (unlike every other tool this skill uses so far, which are MCP calls) — `/wavemachine`'s own pre-flight (`command -v wave-status generate-status-panel mcp-log`) is the precedent if this ever needs to be a hard gate here too; today `|| true` degrades to "no card pinned," reported, not a refusal.
