...
Narration: "The flow is: the per-wave Dynamic Workflow creates `kahuna/<wave-id>`, then runs the wave's Stories as parallel flight agents — each in its own worktree off kahuna. Flights implement, run precheck, and the Workflow's reconcile stage merges them to kahuna — not main. When the wave is assembled, the Workflow's trust gate computes the four signals against kahuna; if they clear, it promotes kahuna→`main`. If not, the wave is held for review. `main` is the protected production branch; kahuna is the sandbox where a wave gets to be wrong before it gets to be right. The control flow is deterministic JS — there is no Orchestrator or Prime agent; the Workflow runtime owns the fan-out."
