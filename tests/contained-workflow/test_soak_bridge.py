...
* mapping — a running dogfood session becomes the correct observation shape; a
  non-running session is skipped; ``active_since`` is the aoe ``created_at`` with a
  transcript-earliest fallback;
* accrual — the full ``run`` accrues dogfood, EXCLUDES dev-mode (R-22) and broken
  (§4.3), and is watermark-idempotent across passes;
...
def test_running_dogfood_maps_to_correct_observation():
    ...
    session, profile, active_since=created_at, active_until=now, broken=False."""
...
def test_active_since_is_the_aoe_created_at():
    """active_since is the session's aoe created_at (the session-start time)."""
...
def test_missing_created_at_and_no_transcript_yields_none(tmp_path):
    """No created_at AND no resolvable transcript ⇒ active_since=None; the observation
    is still emitted, and soak_ledger will skip it with 'unparseable active span' (a
    visible reason, never a silent zero)."""
    (obs,), _ = bridge.build_observations(
        [_assessment("z", "dogfood", "running")],
    ...
    assert obs["active_since"] is None
    results = bridge.sl.accrue(observations=[obs], ledger_path=tmp_path / "l.jsonl", now=NOW)
    assert not results[0].accrued and "unparseable" in results[0].reason
...
def test_run_accrues_dogfood_excludes_dev_mode_and_broken(tmp_path):
    ...
    assert ledger_records[0]["hours"] == 30.0  # created_at defaults to NOW-30h
...
def test_run_is_watermark_idempotent_across_passes(tmp_path):
    ...
    _, _, first = bridge.run(
        ledger_path=ledger, now=NOW, surgeon_runner=_runner([assessment]), aoe_runner=_runner([session])
    )
