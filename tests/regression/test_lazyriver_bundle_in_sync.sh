#!/usr/bin/env bash
# test_lazyriver_bundle_in_sync.sh — #844: single-file /lazyriver Workflow bundle integrity + no-drift.
#
# Picked up by scripts/ci/validate.sh's "Regression tests" loop (tests/regression/*.sh).
# A Dynamic Workflow MUST be ONE self-contained file with `export const meta` first. The goal-seek
# loop's tested halves (schemas + prompt builders + cordCheck) live in river.js; bundle.mjs inlines
# them into lazyriver-workflow.bundled.js — the artifact the Workflow tool runs. This test pins that
...
LR="skills/lazyriver"
BUNDLE="$REPO_DIR/$LR/lazyriver-workflow.bundled.js"
fail=0

echo "test_lazyriver_bundle_in_sync"
echo "──────────────────────────────────────────"
...
# 1. bundle exists
if [[ ! -f "$BUNDLE" ]]; then
	echo "  [FAIL] $LR/lazyriver-workflow.bundled.js missing — run: node $LR/bundle.mjs"
	exit 1
fi

# 2. in sync with source modules
if node "$REPO_DIR/$LR/bundle.mjs" --check &>/dev/null; then
	echo "  [PASS] bundle in sync with source modules"
else
	echo "  [FAIL] bundle is stale — run: node $LR/bundle.mjs"
	fail=1
fi
